...
user_data = {
    ...
    'medicines': ['Jardiance 10mg', 'Pradaxa 150mg', 'Entresto 97-103mg',
                  'Amlodopine 5mg', 'Vitamin B12 1000 mcg', 'Glipizide 10mg',
                  'Omega 3 Ethyl Esters Caps', 'Metformin 500mg', 'Vitamin C 500mg', 
                  'Alfuvosin 10mg', 'Rosuvastatin 20mg', 'Spironolactone 25mg', 
                  'Centrum Silver Men 50+ Tab', 'Fenofibrate 145mg', 'OmeprazoleO 40mg', 
                  'Solatol 80mg']
}
...
def prepare_patient_context(data):
    ...
    context +=f"\n**Medicines History: {data['medicines']}**\n"
    return context
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("🔄 New Conversation"):
        # Delete the messages key instead of setting to None
...
# Display chat history (now messages is guaranteed to exist and not be None)
for msg in st.session_state.messages:
    if msg["role"] != "system":
        with st.chat_message(msg["role"]):
            st.markdown(msg["content"])
